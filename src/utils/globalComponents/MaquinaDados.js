...
export default function MaquinaDados({maqNumber, setMaquinasDosContrato, maquinasDoContrato}) {
  ...
  function adicionarNumeroSerieEContador(e){
    ...
    if (serial !== "" && contador !== 0 && modelo !== ""){
        ...
        toast.error("Para salvar, vc precisa digitar todos os dados necessários...", {
  ...
  return (
    <Container controlInput={controlInput}>
      ...
      <ToastContainer />
        ...
        <ColunaB>
            ...
            <label>DIGITE O MODELO:</label>
                <input
                onBlur={() => window.scrollTo({ top: 5000, behavior: "smooth" })}
                autoFocus
                type="text"
                placeholder="EX.: MULTIFUNCIONAL SAMSUNG 6555NX"
                minLength={5}
            ...
            <label>DIGITE O NUMERO DE SÉRIE:</label>
                ...
                <input
                onBlur={() => window.scrollTo({ top: 10000, behavior: "smooth" })}
                type="number"
                disabled={controlInput ? true : false}
...
position: relative;
    ...
    padding: 20px;
    display: flex;
    border:${props => props.controlInput ? "4px solid green": ""};
